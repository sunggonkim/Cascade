...
## 📊 3-Tier Storage Performance (Job 48443195)

**환경**: A100-SXM4-40GB, 512MB 블록, 10 반복, Cold Read (posix_fadvise)

```
┌──────────────────────────────────────────────────────────────────────────────┐
│                    TIER 1: GPU VRAM (GB/s)                                   │
├──────────────────────────────────────────────────────────────────────────────┤
│ GPU D2D (vLLM native) ████████████████████████████████████████    652.8     │
│ NVLink P2P (GPU→GPU)  ██████                                       93.7     │
│ H2D PCIe (CPU→GPU)    ██                                           26.8     │
│ D2H PCIe (GPU→CPU)    ██                                           26.3     │
├──────────────────────────────────────────────────────────────────────────────┤
│                    TIER 2: SHM/DRAM (GB/s)                                   │
├──────────────────────────────────────────────────────────────────────────────┤
│ torch pinned mem      █                                            13.3     │
│ np.memmap             ▌                                             5.1     │
│ Python buffered I/O   ▌                                             6.2     │
├──────────────────────────────────────────────────────────────────────────────┤
│                    TIER 3: Lustre Cold Read (GB/s)                           │
├──────────────────────────────────────────────────────────────────────────────┤
│ HDF5                  ▌                                             5.2     │
│ Python I/O            ▌                                             4.7     │
│ torch.save/load       ▌                                             4.6     │
...
| Tier | Best Method | Bandwidth | vs Lustre |
|------|-------------|-----------|-----------|
| **GPU VRAM (Tier 1)** | D2D copy | **652.8 GB/s** | **126× faster** |
| **SHM (Tier 2)** | torch pinned | **13.3 GB/s** | **2.6× faster** |
| **Lustre (Tier 3)** | HDF5 | 5.2 GB/s | 1× (baseline) |

> **결론**: Hot KV cache는 GPU에 유지 (126× faster), Warm은 SHM (2.6× faster)
